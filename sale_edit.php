...
<section class="card narrow">
  ...
  <form method="post" class="stack sale-form" enctype="multipart/form-data">
    ...
    <div class="photo-edit-block"><p class="form-label">売上明細・伝票写真</p><?php if (!empty($sale['document_path'])): ?><div class="current-photo"><img class="diary-photo-preview" src="<?= e($sale['document_path']) ?>" alt="現在の売上明細・伝票写真"></div><label class="checkbox-label"><input type="checkbox" name="delete_document" value="1"> 現在の明細写真を削除する</label><?php else: ?><p class="description">現在、明細写真はありません。</p><?php endif; ?><label>新しい売上明細・伝票写真<input type="file" name="document" accept="image/jpeg,image/png,image/webp"><span class="description">新しい写真を選ぶと差し替えます。JPG / JPEG / PNG / WEBP、最大3MB。</span></label></div>
    <label>メモ<textarea name="memo" rows="4"><?= e($memo) ?></textarea></label>
    <div class="button-row"><button class="primary" type="submit">更新する</button><a class="btn" href="sale_detail.php?id=<?= (int)$id ?>">詳細へ戻る</a><a class="btn" href="sale_list.php">一覧へ戻る</a></div>
  </form>
</section>
<script>
(function(){
